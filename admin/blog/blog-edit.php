...
if (strlen($_SESSION['alogin']) == 0) {
    ...
    if (isset($_POST['blogeditBTN'])) {
        if (($_FILES["blogimg"]["error"]) == '4' && ($_FILES["thumb"]["error"]) == '4') { //without two images
            ...
            $sql = "UPDATE blog SET title=?,content=?,autherName=?,image=?,thumbnail=?,slug=? where id=?";
            // print_r($sql);
            // exit();
            $query = $dbh->prepare($sql);
            $result = $query->execute([$title, $content, $authername, $blogImg, $thumb, $slug, $id]);
            ...
            $sql = "UPDATE blog SET title=?,content=?,autherName=?,image=?,thumbnail=?,slug=? where id=?";
            $query = $dbh->prepare($sql);
            $result = $query->execute([$title, $content, $authername, $blogimage, $thumb, $slug, $id]);
            if ($query->rowCount() > 0) {
                echo '<script>alert("Success")</script>';
                echo '<script>window.location = "blog.php";</script>';
            } else {
...
<head>
    <title>Admin Edit Blog | Infinio Technology Solutions</title>
    <meta charset="utf-8">
